...
export const experience = [
  {
    ...
    achievements: [
      ...
      "Built Employee Directory & Org Hierarchy module: manager/skip-level chain resolution, rater auto-assign, 360 Score Calculation Engine (deterministic L1-L4 scoring) with statistical fairness/bias flagging, Calibration Dashboard, and IDP lifecycle — grew from 65 to 81 real-data E2E specs",
      "Designed and built the Candidate Share & Reviewer workflow from scratch in 2020 (token-based access for reviewers with no company account, voting, encrypted PII) — then personally led two more full redesigns in 2025 and 2026, extending it to internal hiring-team reviewers, full candidate anonymization, and a swipe-based quick-review mode, on the same underlying data across all three generations",
      "Built the candidate list's multi-view system: a search-engine-backed Grid view plus a Table view with nested, Jira-style dynamic filters (v1) — extended to a third Kanban/Board view and save-able named views (column selection + filter combination) on the rewrite, as primary engineer on both backend and frontend across both generations",
